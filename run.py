...
vocab_new = "Vocab/vocab_new.txt"    #words that have at least a 50% success rate
...
def generateRandomPack(num):
    ...
    words_new = readVocabFile(vocab_new)
    ...
    for i in list(range(numnew)):
        index = randint(0, len(words_new)-1)
        pack.append(words_new.pop(index))


    writeVocabFile(vocab_temp, pack)
    writeVocabFile(vocab_good, words_good)
    writeVocabFile(vocab_bad, words_bad)
    writeVocabFile(vocab_new, words_new)
...
def fileWordsToFiles(pack):
    ...
    fbad = open(vocab_bad, "a+")
    fgood = open(vocab_good, "a+")
    fnew = open(vocab_new, "a+")
